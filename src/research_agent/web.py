...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            api_key = str(body.get("api_key", "")).strip() or os.environ.get("OPENAI_API_KEY", "")
            if not demo and not api_key:
                raise ValueError("Enter an OpenAI API key or select offline demo.")
        ...
        threading.Thread(target=_run_agent, args=(run_id, goal, demo, model, run_dir, api_key), daemon=True).start()
...
def _run_agent(run_id: str, goal: str, demo: bool, model: str, run_dir: Path, api_key: str) -> None:
    with RUNS_LOCK:
    ...
    try:
        ...
        with RUNS_LOCK:
            RUNS[run_id].update(status=state.status, answer=state.final_answer, steps=state.step)
...
<section class="card"><form id="form"><label for="goal">Research goal</label><textarea id="goal" required>Compare checkpointing and retry logic for resilient agents.</textarea><div class="row"><div><label for="model">Model</label><input id="model" value="gpt-5-mini"></div><div><label class="check"><input id="demo" type="checkbox" checked> Offline demo</label></div></div><div id="keyRow" class="hidden"><label for="apiKey">OpenAI API key · used for this run only</label><div class="keyline"><input id="apiKey" type="password" autocomplete="off" spellcheck="false" placeholder="Enter key"><button id="reveal" class="secondary" type="button" aria-label="Show API key">Show</button></div><p class="hint">Never written to checkpoints, traces, browser storage, or server responses.</p></div><button id="run">Run agent</button></form><p id="formError" class="error"></p></section>
<section id="result" class="card hidden"><div class="status"><strong>Run progress</strong><span id="status" class="pill">starting</span></div><h2>Answer</h2><div id="answer" class="sub">Working…</div><h2>Trace</h2><div id="events"></div></section></main>
<script>
const form=document.querySelector('#form'),button=document.querySelector('#run'),result=document.querySelector('#result'),statusEl=document.querySelector('#status'),answer=document.querySelector('#answer'),events=document.querySelector('#events'),formError=document.querySelector('#formError'),goalInput=document.querySelector('#goal'),modelInput=document.querySelector('#model'),demoInput=document.querySelector('#demo'),keyRow=document.querySelector('#keyRow'),apiKey=document.querySelector('#apiKey'),reveal=document.querySelector('#reveal');
