...
def _run_agent(run_id: str, goal: str, demo: bool, provider: str, model: str, run_dir: Path, api_key: str) -> None:
    ...
    try:
        if demo:
            ...
            planner = GroqPlanner(model, api_key=api_key) if provider == "groq" else OpenAIPlanner(model, api_key=api_key)
            tools = HttpTools()
...
providerInput.addEventListener('change',()=>{const groq=providerInput.value==='groq';modelInput.value=groq?'openai/gpt-oss-20b':'gpt-5-mini';keyLabel.textContent=(groq?'Groq':'OpenAI')+' API key · used for this run only'});
