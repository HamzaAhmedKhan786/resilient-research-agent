...
def run_case(name: str, plan: list[Action], failures: dict[str, int] | None = None, max_steps: int = 8) -> Result:
    run_dir = ROOT / ".runs" / "evals" / name
    if run_dir.exists():
        shutil.rmtree(run_dir)
    tools = LocalCorpusTools(ROOT / "evals" / "corpus")
    wrapped = FlakyTools(tools, failures or {})
    state = ResearchAgent(ScriptedPlanner(plan), wrapped, run_dir, max_steps=max_steps).run("Explain why checkpointing helps.")
...
def main() -> int:
    results = [
        ...
        run_case("invalid_citation_recovery", actions(False) + [Action("finish", {"answer": "Durable checkpoints avoid repeated work [S1]."})]),
    ]
